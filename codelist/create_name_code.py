...
with open(f'{dir}/../raw/zkn_form_ja_modified.json', encoding='UTF-8') as fin:
    dict = json.load(fin)
    forms = {}
    for key in dict['zkn_form'].keys():
        ...
        if id not in forms:
            forms[id] = {}
        forms[id][form_num] = modify_form_name(dict['zkn_form'][key])
...
for name in names:
    ...
    for key in dict[str(id)]:
        dict[str(id)][key] = zukan_name(dict[str(id)][key])

    print(id, dict[str(id)])
    id += 1

with open(f'{dir}/name_code.json', 'w', encoding='UTF-8') as fout:
